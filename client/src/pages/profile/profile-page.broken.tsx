...
import {
  ...
  Bell, HelpCircle, MessageSquare, Flag
} from "lucide-react";
...
export default function ProfilePage() {
  ...
  const handleBannerPhotoChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    if (!event.target.files || event.target.files.length === 0) {
      return;
    }
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to upload banner photo. Please try again.",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
  ...
  return (
    <div className="flex flex-col bg-white min-h-screen">
      {/* Left sidebar and main content wrapper */}
      <div className="flex min-h-screen">
        ...
        <div className="w-[230px] border-r h-screen sticky top-0 flex flex-col bg-white shadow-sm overflow-y-auto">
          ...
          <div className="px-3 flex-1 overflow-y-auto">
            <button
              className="w-full flex items-center px-4 py-2.5 text-left rounded-md transition-all duration-200 my-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#09261E]/50 bg-[#09261E]/10 text-[#09261E] font-medium shadow-sm"
            >
            ...
            <div className="mt-4">
              ...
              <div className="ml-8 mt-1 mb-2">
                <ProfileMenuItem
                  ...
                  active={location === "/help/suggestions"}
                />
                <ProfileMenuItem
                  icon={<Flag size={16} />}
          ...
          {/* Logout */}
          <div className="px-3 pb-5 pt-3 border-t">
            <button
            ...
            {/* Add avatar to bottom left corner of sidebar */}
            <div className="flex items-center mt-4 px-4">
              ...
              <span className="text-sm text-gray-600">@{profileData.username}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 bg-gray-50/60 overflow-y-auto">
          {/* Banner section */}
          <div className="relative h-48 md:h-64 w-full bg-gradient-to-r from-[#09261E]/90 to-[#09261E]/70 overflow-hidden">
            {profileData.profile_banner_url && (
              <div 
                className="absolute inset-0 bg-cover bg-center" 
                style={{ backgroundImage: `url(${profileData.profile_banner_url})` }}
              ></div>
            )}
            <div className="absolute inset-0 bg-black/20 flex items-center justify-center">
              <Button
                ...
                variant="outline"
                size="sm"
                className="bg-white/80 hover:bg-white text-black border-white/50 backdrop-blur-sm"
                onClick={() => bannerInputRef.current?.click()}
              >
                <Upload className="h-4 w-4 mr-2" />
                {profileData.profile_banner_url ? "Change Banner" : "Add Banner Image"}
              </Button>
              <input 
                ref={bannerInputRef}
                type="file"
